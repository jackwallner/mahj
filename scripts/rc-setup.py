#!/usr/bin/env python3
"""Wire Bridge App Store products into RevenueCat.

Usage: RC_SECRET_KEY=sk_... python3 scripts/rc-setup.py
"""
...
BUNDLE_ID = "com.jackwallner.bridge"
PRODUCTS = (
    ("com.jackwallner.bridge.monthly", "Monthly", "subscription", "$rc_monthly"),
    ("com.jackwallner.bridge.yearly", "Yearly", "subscription", "$rc_annual"),
    ("com.jackwallner.bridge.lifetime", "Lifetime", "one_time", "$rc_lifetime"),
)


def request(method: str, path: str, body: dict | None = None) -> dict:
    key = os.environ.get("RC_SECRET_KEY")
    if not key:
        raise SystemExit("error: set RC_SECRET_KEY")
...
def main() -> None:
    ...
    project = next(project for project in projects if project["name"].lower() == "bridge")
    ...
    entitlement = next(
        (
            ...
            if entitlement["lookup_key"] in {"pro", "Bridge+"}
        ),
        None,
    )
    if entitlement is None:
        entitlement = request(
            ...
            {"lookup_key": "pro", "display_name": "Bridge+"},
        )
