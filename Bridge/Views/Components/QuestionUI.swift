...
struct ChoiceList: View {
    ...
    private func row(_ index: Int) -> some View {
        ...
        return Button {
            ...
            HStack {
                ...
                if answered {
                    if isAnswer {
                        ...
                        Image(systemName: "xmark.circle.fill").foregroundStyle(Theme.errorRed)
                    }
                }
            }
            .padding(16)
            ...
            .winGlow(Theme.successGreen, active: answered && isAnswer && landed)
    ...
    private func background(_ index: Int) -> Color {
        ...
        if index == answerIndex { return Theme.successGreen.opacity(0.18) }
        if index == selection { return Theme.errorRed.opacity(0.15) }
        return Theme.card
    }

    private func border(_ index: Int) -> Color {
        guard answered else { return Theme.rule }
        if index == answerIndex { return Theme.successGreen.opacity(0.6) }
        if index == selection { return Theme.errorRed.opacity(0.5) }
        return Theme.rule
    }
}
